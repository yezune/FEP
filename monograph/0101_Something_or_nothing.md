...
이 표준 형식을 사용하면(Yuan 등, 2010), $p(x) = exp(-\Im(x))$가 포커-플랑크 방정식의 해임을 간단하게 보여줄 수 있습니다(Friston and Ao, 2012). 정보 이론에서 스칼라 퍼텐셜 $\Im(x) = -\ln p(x)$는 자기 정보, _놀라움_ 또는 더 간단하게는 놀라움(surprise)으로 알려져 있습니다(Jones, 1979; Tribus, 1961). 이는 우리가 비평형 안정상태 밀도 또는 놀라움에 따라 흐름을 표현할 수 있음을 의미하며, 이는 (부록 B)의 비평형 안정상태 정리와 (Friston, 2013)에 따릅니다:
