...
여기서 기대치는 비평형 안정상태에서 입자나 에이전트들의 앙상블에 대한 것입니다. 이것은 앙상블의 각 구성원이 자신의 변분 자유 에너지를 최소화하려고 한다면, 앙상블의 열역학적 엔트로피도 최소화될 것이라는 흥미로운 함축을 가지고 있습니다. (Friston 등, 2015a)에서 이 앙상블 관점이 어떻게 앙상블의 (모의된) 세포에서 패턴 형성과 형태형성을 시뮬레이션하는 데 사용될 수 있는지에 대한 예를 보십시오.
#### 요약
